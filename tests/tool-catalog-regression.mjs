...
try {
  ...
  for (const viewport of [{width:1280,height:900},{width:390,height:844}]) {
    ...
    if(viewport.width===1280){
      ...
      await page.getByLabel('Nombre de la categoría *',{exact:true}).fill('Compresores');
      await page.getByRole('button',{name:'Crear categoría',exact:true}).last().click();
      await page.getByText('Catálogo actualizado',{exact:true}).waitFor();
      assert.ok(registry.some(r=>r.name==='Compresores'));
      await page.getByLabel('Categoría principal',{exact:true}).selectOption('Escalera');
      await page.getByRole('button',{name:'Crear subcategoría',exact:true}).click();
      await page.getByRole('dialog').getByLabel('Nombre de la subcategoría *',{exact:true}).fill('16 peldaños');
      await page.getByRole('button',{name:'Crear subcategoría',exact:true}).last().click();
      await page.getByText('Catálogo actualizado',{exact:true}).waitFor();
