...
try {
  ...
  assert.equal(await page.getByLabel('Categoría', {exact:true}).count(), 0, 'Only the subcategory filter remains');
  assert.equal(await page.getByLabel('Subcategoría', {exact:true}).locator('option').filter({hasText:/^Amoladora · 9 pulgadas/}).count(), 0, 'No empty standard variants');
  await page.getByLabel('Subcategoría', {exact:true}).selectOption(JSON.stringify(['Amoladora', '7 pulgadas']));
  ...
  assert.equal(await page.getByLabel('Subcategoría', {exact:true}).locator('option').filter({hasText:/^Amoladora · 7 pulgadas/}).count(), 0);
