...
El resolvedor anterior (`src/services/geo/tucumanGeoRegistry.ts`) contiene ubicaciones por coincidencia de nombres y una dispersión artificial basada en el ID. Se conserva para facilitar la recuperación del código, pero el mapa de operaciones excluye todos esos resultados referenciales. Solo presenta obras con coordenadas válidas guardadas en la base.
